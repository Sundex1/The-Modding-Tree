var challengeColors = [null, "#FF8800", "#DD00EE", "#0044DD", "AF0000", "0088FF"];
...
addLayer("c", {
    ...
    challenges: {
        ...
        12: {
            ...
            canComplete() { return false },
            fullDisplay() {
                let display = "Challenge: Achievement Rewards (aside from buyable unlocks) are disabled and the Prestige Point effect is set to " + formatWhole((hasAchievement("a", 32) && hasAchievement("a2", 26)) ? D.max(tmp.c.challenges[12].effect2, tmp.p.effect) : 1) +".<br><br>";
                ...
                display += "Effect: +" + formatWhole(tmp[this.layer].challenges[this.id].effect) + " Perma-Goals towards toggled layers.<br>"
                if (hasAchievement("a", 32)) {
                    display += "<br>Reward 2: Increases the hardcap for Overcharge Point effect<br>";
                    display += "Effect 2: +" + formatWhole(tmp[this.layer].challenges[this.id].effect2);
                }
                return display;
            },
            effect() {
        ...
        13: {
            name: "Direct Miss",
            unlocked() { return hasAchievement("a", 38) || player.c3.points.gte(1) || inChallenge("c", 21); },
            style: { "background-color": challengeColors[3], width: '400px', height: '400px', },
            onEnter() {
                player["c" + (this.id - 10)].resetTime = 0;
                player.subtabs["tree-tab"].mainTabs = "Main";
            },
            ...
            canComplete() { return false },
            fullDisplay() {
                let display = "Challenge: Row 3 Buyables are removed, and position reduction rate is rooted based on your position and your Direct Points.<br><br>";
                display += "Points: " + formatWhole(player.c3.points) + "<br><br>";
                display += "Reward: You can bulk buy buyables outside of this challenge based on your Direct Points <br>";
                display += "Effect: +" + formatWhole(tmp[this.layer].challenges[this.id].effect) + " to bulk buy.<br>";
                if (hasAchievement("a3", 12)) {
                    display += "Reward 2: Direct Points Directly subtract from Prestige Upgrade costs <br>";
                    display += "Effect 2: -" + formatWhole(tmp[this.layer].challenges[this.id].effect2);
                    }
                return display;
            },
    ...
    buyables: {
        ...
        21: {
            ...
            cost(x) {
                ...
                if (hasAchievement("a1", 18)) {
                    if (hasAchievement("a", 43)) cost = cost.div(player.c1.buyables[11].plus(1).log(2).plus(1)); else cost = cost.div(player.c1.buyables[11].plus(1).log10().plus(1));
                };
        ...
        22: {
            ...
            cost(x) {
                ...
                if (hasAchievement("a", 18)) cost = cost.div(player[this.layer].position.log10().times(hasAchievement("a1", 14)?player.c1.points.plus(1):1).times(-1).plus(1));
                if (hasAchievement("a1", 18)) {
                    if (hasAchievement("a", 43)) cost = cost.div(player.c1.buyables[12].plus(1).log(2).plus(1)); else cost = cost.div(player.c1.buyables[12].plus(1).log10().plus(1));
                };
                if (hasAchievement("a1", 26) && hasAchievement("a", 14)) cost = cost.div(Math.max(D(player.a.achievements.length, 1).times(hasAchievement("a3", 14) ? Math.max(player.a1.achievements.length, 1) : 1))); 
                return cost;
            },
        ...
        23: {
            ...
            cost(x) {
                ...
                if (hasAchievement("a", 18)) cost = cost.div(player[this.layer].position.log10().times(hasAchievement("a1", 14)?player.c1.points.plus(1):1).times(-1).plus(1));
                if (hasAchievement("a1", 18)) {
                    if (hasAchievement("a", 43)) cost = cost.div(player.c1.buyables[13].plus(1).log(2).plus(1)); else cost = cost.div(player.c1.buyables[13].plus(1).log10().plus(1));
                };
                if (hasAchievement("a1", 26) && hasAchievement("a", 14)) cost = cost.div(Math.max(D(player.a.achievements.length, 1).times(hasAchievement("a3", 14) ? Math.max(player.a1.achievements.length, 1) : 1))); 
                return cost;
            },
        ...
        31: {
            title() { return "Direct Attack<br>[" + formatWhole(player[this.layer].buyables[this.id]) + (tmp[this.layer].buyables[this.id].extraLevels.gt(0) ? (" + " + formatWhole(tmp[this.layer].buyables[this.id].extraLevels)) : "") + "]" },
            ...
            effect2(x=player[this.layer].buyables[this.id]) {
                return x.plus(1).plus(tmp[this.layer].buyables[this.id].extraLevels).log2().plus(1);
            },
            canAfford() { return player[this.layer].points.gte(layers[this.layer].buyables[this.id].cost(player[this.layer].buyables[this.id]))},
            buy() {
                player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(Math.max(tmp.c.challenges[13].effect, 1));
        ...
        32: {
            title() { return "Shrink Factor<br>[" + formatWhole(player[this.layer].buyables[this.id]) + (tmp[this.layer].buyables[this.id].extraLevels.gt(0) ? (" + " + formatWhole(tmp[this.layer].buyables[this.id].extraLevels)) : "") + "]" },
            ...
            effect(x = player[this.layer].buyables[this.id]) {
                return player.c1.position.log10().times(-1).plus(1).pow(x.plus(tmp[this.layer].buyables[this.id].extraLevels).div(3)).times((hasAchievement("a1", 24))? player.b.points.plus(player.c1.points).plus(player.c2.points).plus(1).log10().plus(1) : 1);
            },
            canAfford() { return player[this.layer].points.gte(layers[this.layer].buyables[this.id].cost(player[this.layer].buyables[this.id])) },
            buy() {
                player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(Math.max(tmp.c.challenges[13].effect, 1));
...
addLayer("c2", {
    ...
    buyables: {
        ...
        11: {
            ...
            cost(x) {
                let total = ((player.p.buyables[41].gte(3)) ? x : x.plus(player[this.layer].buyables[12].plus(player[this.layer].buyables[13])).sub(tmp[this.layer].buyables[21].effect));
                let cost = Decimal.pow(1.1, total).div(10.5);
                return cost;
            },
        ...
        12: {
            ...
            cost(x) {
                let total = ((player.p.buyables[41].gte(3)) ? x : x.plus(player[this.layer].buyables[11].plus(player[this.layer].buyables[13])).sub(tmp[this.layer].buyables[22].effect));
                let cost = Decimal.pow(1.1, total).div(21);
                return cost;
            },
        ...
        13: {
            ...
            cost(x) {
                let total = ((player.p.buyables[41].gte(3)) ? x : x.plus(player[this.layer].buyables[11].plus(player[this.layer].buyables[12])).sub(tmp[this.layer].buyables[23].effect));
                let cost = Decimal.pow(1.1, total).div(31.5);
                return cost;
            },
...
addLayer("c3", {
    name: "Direct Points", // This is optional, only used in a few places, If absent it just uses the layer id.
    symbol: "C3", // This appears on the layer's node. Default is the id with the first letter capitalized
    position: 0, // Horizontal position within a row. By default it uses the layer id and sorts in alphabetical order
    startData() {
        return {
            unlocked: true,
    ...
    requires: new Decimal(.1), // Can be a function that takes requirement increases into account
    resource: "Direct Points", // Name of prestige currency
    baseResource: "position", // Name of resource prestige is based on
    baseAmount() { return player[this.layer].position }, // Get the current amount of baseResource
    type: "custom", // normal: cost to gain currency depends on amount gained. static: cost depends on how much you already have
    base() { return D(.1) },
    ...
    buyables: {
        ...
        11: {
            ...
            unlocked: true,
            display() {
                return "Divides the position<br><br>Req: " + format(tmp[this.layer].buyables[this.id].cost) + " Direct Points<br><br>Currently: /" + format(tmp[this.layer].buyables[this.id].effect)
            },
            cost(x) {
                let total = ((player.p.buyables[41].gte(3)) ? x : x.plus(player[this.layer].buyables[12].plus(player[this.layer].buyables[13])).sub(tmp[this.layer].buyables[21].effect));
                let cost = Decimal.pow(1.1, total).div(10.5);
        ...
        12: {
            ...
            unlocked: true,
            display() {
                return "Exponentiates the position<br><br>Req: " + format(tmp[this.layer].buyables[this.id].cost) + " Direct Points<br><br>Currently: ^" + format(tmp[this.layer].buyables[this.id].effect)
            },
            cost(x) {
                let total = ((player.p.buyables[41].gte(3)) ? x : x.plus(player[this.layer].buyables[11].plus(player[this.layer].buyables[13])).sub(tmp[this.layer].buyables[22].effect));
                let cost = Decimal.pow(1.1, total).div(21);
            ...
            effect(x) { return D(x).plus(1).log(10).div(2).plus(1) },
            canAfford() { return player[this.layer].points.gte(layers[this.layer].buyables[this.id].cost(player[this.layer].buyables[this.id])) },
            buy() {
                player[this.layer].buyables[this.id] = player[this.layer].buyables[this.id].plus(1);
            },
        ...
        13: {
            ...
            unlocked: true,
            display() {
                return "Multiplies the effective level of Anti-Velocity<br><br>Req: " + format(tmp[this.layer].buyables[this.id].cost) + " Direct Points<br><br>Currently: " + format(tmp[this.layer].buyables[this.id].effect) + "x";
            },
            cost(x) {
                let total = ((player.p.buyables[41].gte(3)) ? x : x.plus(player[this.layer].buyables[11].plus(player[this.layer].buyables[12])).sub(tmp[this.layer].buyables[23].effect));
                let cost = Decimal.pow(1.1, total).div(31.5);
        ...
        21: {
            ...
            unlocked: true,
            display() {
                return "Reduces the cost of Anti-Velocity<br><br>Req: " + format(tmp[this.layer].buyables[this.id].cost) + " Direct Points<br><br>Currently: -" + format(tmp[this.layer].buyables[this.id].effect);
            },
            cost(x) {
                let total = x.plus((tmp.p.buyables[22].effect.gte(2)) ? 0 : player[this.layer].buyables[22].plus(player[this.layer].buyables[23]));
                let cost = D.pow(1.25, total);
        ...
        22: {
            ...
            unlocked: true,
            display() {
                return "Reduces the cost of Anti-Acceleration<br><br>Req: " + format(tmp[this.layer].buyables[this.id].cost) + " Direct Points<br><br>Currently: -" + format(tmp[this.layer].buyables[this.id].effect);
            },
            cost(x) {
                let total = x.plus((tmp.p.buyables[22].effect.gte(2)) ? 0 : player[this.layer].buyables[21].plus(player[this.layer].buyables[23]));
                let cost = D.pow(1.25, total);
        ...
        23: {
            ...
            unlocked: true,
            display() {
                return "Reduces the cost of Angles<br><br>Req: " + format(tmp[this.layer].buyables[this.id].cost) + " Direct Points<br><br>Currently: -" + format(tmp[this.layer].buyables[this.id].effect);
            },
            cost(x) {
                let total = x.plus((tmp.p.buyables[22].effect.gte(2)) ? 0 : player[this.layer].buyables[22].plus(player[this.layer].buyables[21]));
                let cost = D.pow(1.25, total);
